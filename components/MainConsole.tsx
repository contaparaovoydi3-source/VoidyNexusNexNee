...
import { Message, Character, ChatSession, UserProfile, Notification, FeedPost, MarketItem, MissionPhase, Community, PlayerState, RpgTest, LocationConnection, RpgWorldEvent, RpgPhase, RpgEnemy } from '../types';
...
export default function MainConsole({ 
  ...
  verifySafety, onDeleteSession, onBlockUser, onAddMemberClick, globalPosts, onUpdateSession, onMinimizeMedia, isBlocked, community 
}: MainConsoleProps) {
    ...
    session.messages.forEach(m => {
      if (m.personaName && m.personaName !== 'SISTEMA' && m.personaName !== character.name) {
        authors.set(m.personaName, {
          ...
          personaAvatar: m.image || `https://api.dicebear.com/7.x/identicon/svg?seed=${m.personaName}`,
          cityRank: 'MEMBRO'
        });
      }
    });
...
Fazer parecer que o jogador foi surpreendido pelo inimigo.`;
  ...
  const saveToMediaStorage = (base64: string): string => {
    const id = `vimg_chat_${Math.random().toString(36).substring(2, 9)}`;
    try {
  ...
  const handleSendMessage = async (textOverride?: string) => {
    ...
    // If user sends an image and mentions keywords related to profile/avatar, update session avatar
    // For Nexus specifically, we treat any image sent as a potential profile update (as requested)
    const isBotChat = session.type === 'IA' || session.id.startsWith('ia-');
    const isNexus = session.id === 'nexus-default' || session.name === 'NEXUS';
    const uploadKeywords = ['perfil', 'avatar', 'foto', 'foto de perfil', 'upload', 'usa essa', 'sua foto', 'imagem de perfil'];
    const hasUploadIntent = uploadKeywords.some(k => currentInput.toLowerCase().includes(k));

    if (currentImage && (hasUploadIntent || isNexus) && onUpdateSession) {
      const savedRef = saveToMediaStorage(currentImage);
      onUpdateSession({ avatar: savedRef });
      
      // Specially persist Nexus custom avatar if this is the Nexus session
      if (isNexus) {
        localStorage.setItem('void_nexus_custom_avatar', savedRef);
        localStorage.setItem('img_NEXUS', savedRef);
        // Force a storage event to refresh icons everywhere
        window.dispatchEvent(new Event('void_storage_update'));
      }

      // Se for chat de IA ou privado, tenta atualizar também o personagem
      if (isBotChat && onUpdateCharacter) {
         onUpdateCharacter({ avatar: savedRef });
      }
    }

    // 2. Handling Narrative Tag Upload (vimg_)
    ...
    if (vimgMatches && currentImage) {
      const vimgId = vimgMatches[1];
      localStorage.setItem(vimgId, currentImage);
      localStorage.setItem(vimgId.toLowerCase(), currentImage);
      localStorage.setItem(vimgId.toUpperCase(), currentImage);
    ...
    const imageToVerify = currentImage; // Base64 data URL
    if (imageToVerify) {
      try {
        localStorage.setItem('void_last_uploaded_img', imageToVerify);
        // Treat as direct "upload" for the AI/Partner if applicable
        if (session.name) {
          const upName = session.name.toUpperCase();
          localStorage.setItem('img_' + upName, imageToVerify);
          // Force immediate session update for visual feedback
          if (session.type === 'IA' || session.type === 'PRIVADO') {
            onUpdateSession?.({ avatar: imageToVerify });
          }
        }
      } catch (e) {}
    }
    ...
    if (verifySafety) {
      ...
      if (imageToVerify) {
        const isSafe = await verifySafety(imageToVerify, 'image');
        if (!isSafe) {
          setInputText('');
          setSelectedImage(null);
          return;
  ...
  return (
    ...
    >
      ...
      {/* HEADER DO CHAT */}
      <header className="absolute top-0 left-0 w-full z-30 px-4 py-4 md:py-6 grid grid-cols-3 items-center bg-transparent">
         ...
         <div className="flex flex-col items-center justify-center">
            <h2 className="text-[11px] font-black text-white uppercase tracking-widest truncate max-w-[150px] md:max-w-[300px] text-center">{session.name}</h2>
          ...
          <div className="flex items-center justify-end gap-1">
            ...
            <button onClick={() => setIsCustomizing(true)} className="p-2 text-slate-400 hover:text-white transition-all active:scale-90">
      ...
      {/* ÁREA DE MENSAGENS */}
      <main className={`flex-1 overflow-y-auto no-scrollbar p-4 ${playingVideo ? 'pt-2 md:pt-2' : 'pt-20 md:pt-24'} pb-28 md:pb-32 space-y-4 relative z-10`}>
        {isMissionChat && (
            ...
            const allLines: { text: string, b?: boolean, i?: boolean, u?: boolean, c?: boolean }[] = 
                                          ...
                                          {parts.map((part, pi) => {
                                             ...
                                             if (vimgIdMatch) {
                                                ...
                                                if (src && src.length > 20) {
                                                   return (
                                                      <div key={pi} className="my-2 max-w-full rounded-lg overflow-hidden border border-white/5 shadow-lg block" onClick={() => setSelectedImage(src)}>
               ...
               return (
                  <div 
                     key={msg.id} 
                     id={`msg-${msg.id}`}
                     className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} w-full animate-in fade-in slide-in-from-bottom-2 duration-300 transition-all ${isCurrentResult ? 'scale-[1.02] ring-2 ring-cyan-500/50 rounded-2xl p-2 bg-cyan-500/5' : ''}`}
                  ...
                  >
                     {!isMe && (
                        <span className="text-[7px] font-black text-cyan-500 uppercase tracking-widest mb-1 ml-11">
                           {msg.personaName || session.name}
                        </span>
                     )}

                     <div className={`flex ${isMe ? 'flex-row-reverse' : 'flex-row'} items-start gap-2 max-w-[85%]`}>
                        {/* Circular Hub */}
                        <div className="w-9 h-9 rounded-full border border-white/10 overflow-hidden shrink-0 shadow-lg bg-black/40">
                           <img 
                        ...
                        <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} relative mt-[18px]`}>
                           {msg.replyToId && (
                              <button 
                           ...
                           >
                                          ...
                                          <p key={i} className={`whitespace-pre-wrap break-words min-h-[1em] ${line.b ? 'font-black text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]' : ''} ${line.i ? 'italic' : ''} ${line.u ? 'underline' : ''} ${line.c ? 'text-center' : ''}`}>
                                             {parts.map((part, pi) => {
                                                ...
                                                if (vimgIdMatch) {
                                                   ...
                                                   if (src && src.length > 20) {
                                                      return (
                                                         <div key={pi} className="my-3 max-w-full rounded-2xl overflow-hidden border border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.5)] cursor-pointer active:scale-95 transition-transform block" onClick={() => setSelectedImage(src)}>
                              ...
                              {msg.image && (
                                 <img 
                                    ...
                                    className={`${isSticker ? 'w-24 h-24 object-contain drop-shadow-md cursor-pointer active:scale-95 transition-transform' : 'mt-3 rounded-xl border border-white/10 w-full max-h-64 object-cover'}`} 
                                    alt="Anexo" 
                                 />
                              )}
                           </div>
            ...
            <div className="relative w-full max-w-[320px] bg-[#0a0c1a] border border-white/10 rounded-[2.5rem] p-6 shadow-2xl flex flex-col gap-2 animate-in zoom-in-95 duration-300">
               {session.type === 'PRIVADO' && (
                  <div className="flex justify-center gap-6 mb-6 mt-2">
                     ...
                     <div className="w-16 h-16 rounded-full border-2 border-cyan-500/50 p-1 shadow-[0_0_20px_rgba(34,211,238,0.2)]">
                        <img src={resolveImageRef(session.avatar) || `https://api.dicebear.com/7.x/identicon/svg?seed=${session.name || 'Nexus'}`} className="w-full h-full rounded-full object-cover" alt="Partner Avatar" referrerPolicy="no-referrer" />
                     </div>
                  </div>
               )}
               {session.type !== 'PRIVADO' && (
                        ...
                        {session.admins?.map((admin) => (
                           <div key={admin} className="flex flex-col items-center gap-1">
                              <div className="w-12 h-12 rounded-full border-2 border-cyan-400 p-0.5 shadow-[0_0_10px_rgba(34,211,238,0.4)]">
                                 <img src={`https://api.dicebear.com/7.x/identicon/svg?seed=${admin}`} className="w-full h-full rounded-full object-cover bg-slate-800" alt="Admin" />
                              </div>
                        ...
                        {session.coAdmins?.map((coAdmin) => (
                           <div key={coAdmin} className="flex flex-col items-center gap-1">
                              <div className="w-10 h-10 rounded-full border border-cyan-400/50 shadow-[0_0_8px_rgba(34,211,238,0.2)] p-0.5 bg-white/5">
                                 <img src={`https://api.dicebear.com/7.x/identicon/svg?seed=${coAdmin}`} className="w-full h-full rounded-full object-cover" alt="Co-Admin" />
                              </div>
             ...
             <div className="flex flex-col items-center gap-6">
                ...
                <div className="w-full space-y-4">
                   ...
                   <div className="space-y-1.5 pt-2">
                      <label className="text-[7.5px] font-black text-cyan-400 uppercase tracking-widest ml-4">Mudar o fundo (Imagem ou Cor)</label>
                      <div className="flex gap-2">
                   ...
                   <div className="space-y-3 pt-2">
                      <label className="text-[7.5px] font-black text-cyan-400 uppercase tracking-widest ml-4">Mudar Balão</label>
                      <div className="grid grid-cols-2 gap-4">
                         <div className="space-y-1">
                            <span className="text-[6px] font-black text-slate-500 uppercase tracking-wider ml-4">Seu Balão</span>
                         ...
                         <div className="space-y-1">
                            <span className="text-[6px] font-black text-slate-500 uppercase tracking-wider ml-4">Balão do Parceiro</span>
                      ...
                      <div className="space-y-2">
                         ...
                         {session.admins?.map(admin => (
                            <div key={admin} className="flex items-center justify-between p-3 bg-white/5 rounded-2xl border border-white/5">
                               <div className="flex items-center gap-3">
                                  <img src={`https://api.dicebear.com/7.x/identicon/svg?seed=${admin}`} className="w-8 h-8 rounded-full bg-slate-800" alt="avatar" />
                                  <div className="flex flex-col">
                      ...
                      <div className="space-y-2">
                         ...
                         {session.coAdmins?.map(coAdmin => (
                            <div key={coAdmin} className="flex items-center justify-between p-3 bg-white/5 rounded-2xl border border-white/5">
                               <div className="flex items-center gap-3">
                                  <img src={`https://api.dicebear.com/7.x/identicon/svg?seed=${coAdmin}`} className="w-8 h-8 rounded-full bg-slate-800" alt="avatar" />
...
const ChatInputModule = React.memo(({ 
  ...
  setIsStickerPickerOpen, isStickerPickerOpen, selectedImage, setSelectedImage, 
  ...
  // Scroll dynamically on typing
  useEffect(() => {
    if (localText && chatEndRef?.current) {
      chatEndRef.current.scrollIntoView({ behavior: 'smooth' });
    }
  }, [localText, chatEndRef]);
  ...
  const resolveImageRef = (ref: string | undefined | null): string | null => {
    if (!ref || typeof ref !== 'string') return null;
    const cleanRef = ref.split('|metadata:')[0];
    if (cleanRef.startsWith('data:') || cleanRef.includes('ais-dev') || cleanRef.includes('googleusercontent') || cleanRef.includes('://') || cleanRef.startsWith('/')) return cleanRef;
    return localStorage.getItem(cleanRef) || `https://api.dicebear.com/7.x/identicon/svg?seed=${cleanRef}`;
  };
  ...
  return (
    <footer className="absolute bottom-0 left-0 w-full z-30 p-4 bg-transparent shrink-0">
         <div className="max-w-4xl mx-auto flex flex-col gap-3 relative">
            ...
            {selectedImage && (
               <div className="relative w-20 h-20 rounded-xl overflow-hidden border border-cyan-500/50 self-end mb-1">
                  <img src={selectedImage} className="w-full h-full object-cover" alt="Preview" />
                  <button 
                     onClick={() => setSelectedImage(null)}
                     className="absolute top-1 right-1 w-5 h-5 bg-black/60 rounded-full flex items-center justify-center text-[10px] text-white"
                  >
                     ✕
                  </button>
               </div>
            ...
            <div className="flex items-center gap-3 bg-black/40 backdrop-blur-xl border border-white/10 p-2 rounded-full">
               ...
               <button 
                  onClick={onSend}
                  disabled={!localText.trim() && !selectedImage}
                  className={`w-11 h-11 rounded-full flex items-center justify-center transition-all active:scale-90 ${localText.trim() || selectedImage ? 'bg-cyan-500 text-black shadow-lg shadow-cyan-500/20' : 'bg-white/5 text-slate-600 cursor-not-allowed'}`}
               >
