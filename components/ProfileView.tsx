...
import { getOrCreateChat, sendMessage } from '../chatService';
...
const resolveImageRef = (ref: string | undefined | null): string | null => {
  ...
  if (cleanRef.toLowerCase().includes('vimg_')) {
    const match = cleanRef.match(/vimg_([a-zA-Z0-9_-]+)/i);
    if (match) {
  ...
  if (cleanRef.toLowerCase().startsWith('img_')) {
    const shortId = cleanRef.replace(/img_/i, '');
    return tryGet(cleanRef) || tryGet('vimg_' + cleanRef) || tryGet('vimg_img_' + shortId) || tryGet(shortId) || null;
  }
...
const formatTags = (text: string, isOverlay = false) => {
  if (!text) return "";
  const tagRegex = /(\[[BbIiCcSsUu]{1,4}\])/g;
  ...
  for (const part of parts) {
    if (part.startsWith('[') && part.endsWith(']')) {
      ...
      if (tagContent.includes('s') || tagContent.includes('u')) currentUnderline = !currentUnderline;
      if (tagContent.includes('c')) currentCentered = !currentCentered;
    } else {
      const segment = part;
      if (segment) {
        const classes = [];
        if (currentBold) classes.push(`font-black ${isOverlay ? 'text-white text-[1.2em] inline-block' : ''}`);
        if (currentItalic) classes.push('italic opacity-90');
        if (currentUnderline) classes.push('underline');
        if (currentCentered) classes.push('text-center block w-full my-1');

        if (classes.length > 0) {
          result += `<span class="${classes.join(' ')}">${segment}</span>`;
        } else {
          result += segment;
        }
      }
...
const ProfileView: React.FC<ProfileViewProps> = ({ 
  onBack, profile, onProfileUpdate, verifySafety, userAvatar, userName, 
  addNotification, onEditPost, onDeletePost, onNavigateToChat 
}) => {
  ...
  const handleFollow = () => {
    ...
    if (!isFollowing) {
      addNotification({
        type: 'FOLLOW',
        title: 'Novo Seguidor',
        content: `${userName} começou a seguir seu perfil global.`,
        sender: userName
      });
    }
  };
  ...
  const handleSendInvite = async () => {
    if (!inviteText.trim() || !profile.id) return;
    setIsCreatingChat(true);
    
    try {
      const room = await getOrCreateChat(profile.id);
      await sendMessage(room.id, inviteText);
      
      addNotification({
        type: 'INVITE',
        title: 'Sinal Enviado',
        content: `Você iniciou uma transmissão com ${profile.name}.`,
        sender: 'NEXUS'
      });
      ...
      setTimeout(() => {
        ...
        setInviteSent(false);
        if (onNavigateToChat) onNavigateToChat(room.id);
      }, 1500);
    } catch (err: any) {
      addNotification({
        type: 'SYSTEM',
        title: 'Erro de Sincronia',
        content: err.message || 'Falha ao estabelecer canal de comunicação.',
        sender: 'VOID'
      });
    } finally {
  ...
  const handleImageInput = (e: React.ChangeEvent<HTMLInputElement>, setter: (val: string) => void, isAvatar = false, isBackground = false) => {
    const target = e.target;
    const file = target.files?.[0];
    if (!file) return;
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        try {
          const canvas = document.createElement('canvas');
          const MAX_SIZE = isAvatar ? 180 : (isBackground ? 960 : 1024);
          let width = img.width;
          let height = img.height;
          if (width > height) {
          ...
          if (ctx) {
            ...
            const mime = base64.split(';')[0]?.split(':')[1]?.split(';')[0] || 'image/jpeg';
            const outputMime = (mime === 'image/png' || mime === 'image/webp') ? mime : 'image/jpeg';
            const compressedBase64 = canvas.toDataURL(outputMime, isAvatar ? 0.72 : 0.82);
            const isDirect = isAvatar || isBackground;
            setter(isDirect ? compressedBase64 : saveToMediaStorage(compressedBase64));
            if (verifySafety) verifySafety(compressedBase64).catch(console.error);
          } else {
            const isDirect = isAvatar || isBackground;
            setter(isDirect ? base64 : saveToMediaStorage(base64));
          }
        } catch (err) {
  ...
  const handleSendMuralComment = () => {
    ...
    const authorName = userName || 'MEMBRO';
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col items-center overflow-y-auto no-scrollbar font-inter text-white select-none hd-4k-rendering transition-colors duration-500" style={{ backgroundColor: activeMainBgColor === 'transparent' ? '#02040a' : activeMainBgColor }}>
      ...
      <input type="file" ref={avatarInputRef} className="hidden" accept="image/*" onChange={(e) => handleImageInput(e, setTempAvatar, true)} />
      <input type="file" ref={hub01InputRef} className="hidden" accept="image/*" onChange={(e) => handleImageInput(e, setTempPanelImage, false, true)} />
      <input type="file" ref={hub02InputRef} className="hidden" accept="image/*" onChange={(e) => handleImageInput(e, setTempContentImage, false, true)} />
      <input type="file" ref={muralWallInputRef} className="hidden" accept="image/*" onChange={(e) => handleImageInput(e, setTempMuralImage, false, true)} />
      <input type="file" ref={muralFeedInputRef} className="hidden" accept="image/*" onChange={(e) => handleImageInput(e, setTempMuralFeedImage, false, true)} />
      
      {/* Wiki Context Inputs */}
      <input type="file" ref={wikiAvatarRef} className="hidden" accept="image/*" onChange={(e) => handleImageInput(e, setWikiAvatar, true)} />
               ...
               {isEditing ? (
                 ...
                 <div className="relative inline-flex items-center">
                   ...
                   {!profile.isMe && !isEditing && (
                     <div className="flex flex-col items-center gap-4 mt-6 w-full max-w-xs animate-in slide-in-from-bottom-4 duration-700 pointer-events-auto">
                        <div className="flex gap-3 w-full">
                           <button 
                             onClick={handleFollow}
                             className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all active:scale-95 shadow-xl cursor-pointer ${isFollowing ? 'bg-white/10 text-white border border-white/20' : 'bg-white text-black shadow-white/20'}`}
                           >
                             {isFollowing ? 'SEGUINDO' : 'SEGUIR'}
                           </button>
                           <button 
                             onClick={() => setShowInviteInput(!showInviteInput)}
                             className="flex-1 py-3 bg-cyan-500 text-black rounded-xl text-[9px] font-black uppercase tracking-widest transition-all active:scale-95 shadow-xl shadow-cyan-500/20 cursor-pointer"
                           >
                             CONVERSAR
                           </button>
                        </div>
                        ...
                        {showInviteInput && (
                          <div className="w-full bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-4 animate-in fade-in zoom-in-95 duration-300 shadow-2xl">
                             {inviteSent ? (
                               <div className="py-2 text-center flex flex-col items-center gap-2">
                                  ...
                                  <span className="text-[8px] font-black text-emerald-400 uppercase tracking-widest">Convite Enviado</span>
                               </div>
                             ) : (
                               <div className="flex flex-col gap-3">
                                  <span className="text-[7px] font-black text-cyan-400 uppercase tracking-widest pl-1">Mensagem de Convite</span>
                                  <textarea 
                                    value={inviteText}
                                    onChange={(e) => setInviteText(e.target.value)}
                                    placeholder="Olá! Vi seu perfil e gostaria de conversar..."
                                    className="w-full bg-white/5 border border-white/5 rounded-xl p-3 text-[10px] text-white outline-none focus:border-cyan-500/40 h-24 resize-none transition-all placeholder:text-slate-600"
                                  />
                                  <div className="flex justify-end gap-2">
                                     <button 
                                       onClick={() => setShowInviteInput(false)}
                                       className="px-3 py-2 text-[7px] font-black text-slate-500 uppercase tracking-widest hover:text-white transition-colors cursor-pointer"
                                     >
                                       Cancelar
                                     </button>
                                     <button 
                                       onClick={handleSendInvite}
                                       disabled={!inviteText.trim() || isCreatingChat}
                                       className="px-5 py-2 bg-cyan-500 text-black rounded-lg text-[8px] font-black uppercase tracking-widest active:scale-95 transition-all shadow-lg shadow-cyan-500/20 disabled:opacity-30 cursor-pointer"
                                     >
                                       {isCreatingChat ? 'CONECTANDO...' : 'Enviar Sinal'}
                                     </button>
                                  </div>
                               </div>
                             )}
