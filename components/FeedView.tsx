...
const resolveImageRef = (ref: string | undefined | null): string | null => {
  ...
  // 1. Data URIs and AIS-generated URLs are generally valid
  if (cleanRef.startsWith('data:') || cleanRef.includes('ais-dev') || cleanRef.includes('googleusercontent') || cleanRef.startsWith('http')) {
    return cleanRef;
  }
  
  // Check if it's a known placeholder for the last uploaded image
  ...
  // 2. Local storage references (ref:, vimg_, img_)
  if (cleanRef.length < 200) {
    const rawVal = tryGet(cleanRef);
    if (rawVal && rawVal.startsWith('data:')) return rawVal;

    const refMatch = cleanRef.match(/(ref:|vimg_|img_)([a-zA-Z0-9_-]+)/i);
    if (refMatch) {
      const fullTag = refMatch[0];
      const id = refMatch[2];
      
      const resolved = tryGet(fullTag) || 
                       tryGet(id) || 
                       tryGet('vimg_' + id) || 
                       tryGet('img_' + id) ||
                       tryGet('vimg_img_' + id) ||
                       tryGet('vimg_' + fullTag);
                        
      if (resolved) return resolved;
    }
  }

  // 3. Fallback for IDs - search in storage first
  if (cleanRef.length < 60) {
     const clean = cleanRef.replace(/^(vimg_|img_)/i, '');
     const resolved = tryGet(cleanRef) || 
                      tryGet('vimg_' + cleanRef) || 
                      tryGet('img_' + cleanRef) || 
                      tryGet('vimg_' + clean) || 
                      tryGet('img_' + clean) ||
                      tryGet('vimg_img_' + clean);
     if (resolved) return resolved;
  }

  // 4. URL/Path detection - avoid potential hijack
  if (cleanRef.includes('://') || cleanRef.startsWith('/')) {
    const stored = tryGet(cleanRef);
    if (stored) return stored;
    return cleanRef;
  }

  // 5. Final fallback
  if (cleanRef.length < 30 && !cleanRef.includes(' ')) {
    return `https://api.dicebear.com/7.x/identicon/svg?seed=${cleanRef}`;
  }
...
const formatTags = (text: string, isOverlay = false) => {
  ...
  for (const part of parts) {
    if (part.startsWith('[') && part.endsWith(']')) {
      ...
      if (segment) {
        const classes = [];
        if (currentBold) classes.push(`font-black text-white ${isOverlay ? 'text-[1.2em] inline-block' : ''}`);
