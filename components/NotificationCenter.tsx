      ...
      <div className="relative ml-auto h-full w-[85%] max-w-[380px] bg-[#050714] border-l border-white/5 flex flex-col shadow-2xl animate-in slide-in-from-right duration-500">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar pb-32">
          {permissionStatus && permissionStatus !== 'granted' && (
            <div className="p-5 rounded-[1.8rem] bg-cyan-950/20 border border-cyan-500/30 text-center space-y-3 shadow-lg flex flex-col items-center">
              <span className="text-xl">🔔</span>
              <div className="space-y-1">
                <h4 className="text-[10px] font-black uppercase text-cyan-400 tracking-widest">Sinalizadores Inativos</h4>
                <p className="text-[9px] text-slate-400 leading-relaxed font-semibold">
                  Autorize os alertas para receber avisos caso receba novas mensagens em segundo plano.
                </p>
                <p className="text-[8px] text-cyan-600 leading-relaxed font-black uppercase">
                  (Se estiver na pré-visualização, abra o app em uma Nova Aba para ativar)
                </p>
              </div>
              <button
                onClick={async () => {
                ...
                className="w-full py-2.5 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-xl text-[9px] font-black text-black uppercase tracking-[0.15em] hover:brightness-110 active:scale-95 transition-all cursor-pointer shadow-md shadow-cyan-500/10"
              >
                Ativar Sinais de Alerta
              </button>
            </div>
          )}
