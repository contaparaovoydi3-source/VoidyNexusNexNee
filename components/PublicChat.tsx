...
const PublicChat: React.FC<PublicChatProps> = ({ onBack, userAvatar, userName, publicMessages, onUpdatePublicMessages, onViewProfile, onAddMemberClick, onLeave, onDeleteSession, addNotification, verifySafety, isSilenced, isAdmin, isCoAdmin }) => {
  ...
  const [showDetailsMessage, setShowDetailsMessage] = useState<Message | null>(null);
        ...
        {publicMessages.map((msg, idx) => {
          ...
          return (
            <div 
              key={msg.id} 
              id={`msg-${msg.id}`}
              className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'} w-full animate-in fade-in duration-300 transition-all ${isCurrentResult ? 'scale-[1.02] ring-2 ring-cyan-500/50 rounded-2xl p-2 bg-cyan-500/5' : ''}`}
            ...
            >
              ...
              <div className={`flex gap-3 ${isOwn ? 'flex-row-reverse' : 'flex-row'} items-start max-w-[85%]`}>
      ...
      {heldMessage && (
        <div key="held-msg-overlay" className="fixed inset-0 z-[1500] flex items-center justify-center p-6 animate-in fade-in duration-300 pointer-events-none">
           ...
           <div className="relative w-full max-w-[280px] bg-[#0a0c1a]/95 border border-cyan-500/30 rounded-[2.5rem] p-6 shadow-2xl flex flex-col gap-2 pointer-events-auto">
              <button onClick={() => { setReplyingTo(heldMessage); setHeldMessage(null); }} className="w-full text-left px-6 py-4 hover:bg-white/5 rounded-2xl text-[10px] font-black uppercase text-white flex items-center gap-3"><span>↩️</span> Responder texto</button>
              
              <button onClick={() => { if (heldMessage.text) navigator.clipboard.writeText(heldMessage.text); setHeldMessage(null); }} className="w-full text-left px-6 py-4 hover:bg-white/5 rounded-2xl text-[10px] font-black uppercase text-white flex items-center gap-3"><span>📋</span> Copiar texto</button>
              
              {heldMessage.personaName === userName && (
                <button onClick={() => { const hId = heldMessage.id; onUpdatePublicMessages(prev => prev.map(m => m.id === hId ? { ...m, text: 'mensagem apagada', isDeleted: true, image: undefined, audio: undefined } : m)); setHeldMessage(null); }} className="w-full text-left px-6 py-4 hover:bg-red-500/10 rounded-2xl text-[10px] font-black uppercase text-red-500 flex items-center gap-3"><span>🗑️</span> Apagar texto</button>
              )}

              <button onClick={() => { setShowDetailsMessage(heldMessage); setHeldMessage(null); }} className="w-full text-left px-6 py-4 hover:bg-white/5 rounded-2xl text-[10px] font-black uppercase text-white flex items-center gap-3"><span>ℹ️</span> Detalhes</button>
           ...
           <div className="relative w-full max-w-[340px] bg-[#02040a] border border-cyan-500/40 rounded-[2rem] p-6 shadow-2xl flex flex-col gap-4 pointer-events-auto font-sans">
              ...
              <div className="space-y-3 text-[11px]">
                <div className="flex justify-between border-b border-white/5 py-1">
                  ...
                  <span className="text-white font-bold">{showDetailsMessage.personaName}</span>
                </div>
                <div className="flex justify-between border-b border-white/5 py-1">
                  <span className="text-slate-500 uppercase font-black text-[8px] tracking-wider font-sans">Data & Hora</span>
                  <span className="text-white font-bold font-mono">{new Date(showDetailsMessage.timestamp).toLocaleString()}</span>
                </div>
                {showDetailsMessage.replyToId && (
                  <div className="flex justify-between border-b border-white/5 py-1">
                    <span className="text-slate-500 uppercase font-black text-[8px] tracking-wider font-sans">Em Resposta a</span>
                    <span className="text-cyan-400 font-bold font-mono">ID: {showDetailsMessage.replyToId.substring(0, 8)}</span>
                  </div>
                )}
                <div className="flex flex-col gap-1 border-b border-white/5 py-1">
                  ...
                  <p className="text-slate-300 bg-white/[0.02] p-3 rounded-xl border border-white/5 font-mono text-[9px] max-h-32 overflow-y-auto whitespace-pre-wrap">{showDetailsMessage.text || "[Imagem / Sticker / Áudio]"}</p>
                </div>
              </div>
              <button onClick={() => setShowDetailsMessage(null)} className="w-full py-3 bg-white/5 border border-white/10 text-[9px] font-black uppercase text-white rounded-2xl hover:bg-white/10 active:scale-95 transition-all text-center">Fechar</button>
           </div>
        </div>
      )}
        ...
        {isAudioMode ? (
          ...
          <div className="flex flex-col w-full gap-2">
            ...
            <form onSubmit={handleSendMessage} className="flex items-center gap-3 bg-[#0a0c1a]/95 backdrop-blur-3xl border border-white/10 rounded-full px-6 py-4 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                 ...
                 <textarea onFocus={() => { setIsStickerOpen(false); setTimeout(() => { chatEndRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }, 120); }} value={inputText} onChange={(e) => { setInputText(e.target.value); }} placeholder="Transmitir sinal..." className="flex-1 bg-transparent border-none outline-none text-white text-[14px] placeholder:text-slate-700 resize-none max-h-32 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden" rows={1} />
