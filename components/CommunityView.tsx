...
const resolveImageRef = (ref: string | undefined | null): string | null => {
  ...
  if (cleanRef.length < 60) {
     ...
     const resolved = tryGet(cleanRef) || tryGet('vimg_' + cleanRef) || tryGet('img_' + cleanRef) || tryGet('vimg_' + clean) || tryGet('img_' + clean) || tryGet('vimg_img_' + clean);
     if (resolved) return resolved;
  }
    ...
    <div className="absolute inset-0 z-[150] flex flex-col overflow-hidden animate-in slide-in-from-left duration-75 font-inter touch-pan-y" style={{ backgroundColor: baseBgColor }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className="absolute bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 w-[85%] md:w-[90%] max-lg h-12 md:h-20 z-[200] flex items-center justify-around px-3 md:px-4 overflow-visible">
            ...
            <div onClick={() => pushView('COMMUNITY_PROFILE')} className="relative z-10 p-0.5 rounded-full border-2 border-slate-300 w-8 h-8 md:w-11 md:h-11 shadow-sm shrink-0 cursor-pointer active:scale-95 transition-all"><img src={resolveImageRef(community.membersData?.[userName]?.personaAvatar || userAvatar) || community.membersData?.[userName]?.personaAvatar || userAvatar || undefined} className="w-full h-full object-cover rounded-full" alt="avatar" /></div>
        </div>
      </div>
