...
const resolveImageRef = (ref: string | undefined | null): string | null => {
  ...
  if (cleanRef.startsWith('data:') || cleanRef.includes('://') || cleanRef.startsWith('/')) {
    return cleanRef;
  }
  ...
  if (cleanRef.length < 50 && !cleanRef.includes(' ') && !cleanRef.includes('/') && !cleanRef.includes('.')) {
    return `https://api.dicebear.com/7.x/identicon/svg?seed=${encodeURIComponent(cleanRef)}`;
  }
          ...
          <section className="animate-in slide-in-from-bottom duration-700">
             ...
             <div className="flex flex-wrap justify-center gap-8">
                {leaders.map(l => (
                  <button 
                    key={l.userId} 
                    onClick={() => onViewProfile({ id: l.userId, name: l.personaName || l.userId, avatarUrl: l.personaAvatar || '', rank: l.cityRank, level: l.cityLevel, isMe: false, reputation: l.cityReputation, following: 0, followers: 0 } as any)} 
          ...
          <section className="animate-in slide-in-from-bottom duration-700 delay-200">
             ...
             <div className="flex flex-wrap justify-center gap-6">
                {coLeaders.map(cl => (
                  <button 
                    key={cl.userId} 
                    onClick={() => onViewProfile({ id: cl.userId, name: cl.personaName || cl.userId, avatarUrl: cl.personaAvatar || '', rank: cl.cityRank, level: cl.cityLevel, isMe: false, reputation: cl.cityReputation, following: 0, followers: 0 } as any)} 
        ...
        <section className="space-y-8 pt-8 border-t border-white/5 animate-in fade-in duration-1000 delay-500">
           ...
           <div className="grid grid-cols-3 gap-x-2 gap-y-10 md:gap-x-6">
              {commonMembers.map((m, idx) => (
                <button 
                  key={m.userId}
                  onClick={() => onViewProfile({ id: m.userId, name: m.personaName || m.userId, avatarUrl: m.personaAvatar || '', rank: m.cityRank, level: m.cityLevel, isMe: false, reputation: m.cityReputation, following: 0, followers: 0 } as any)}
