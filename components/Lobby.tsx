...
import { TEST_COMMUNITIES } from '../data';
...
const resolveImageRef = (ref: string | undefined | null): string | null => {
  ...
  // 1. Check direct URL or data
  if (cleanRef.startsWith('data:') || cleanRef.includes('://') || cleanRef.startsWith('/')) {
    return cleanRef;
  }

  // 2. Check if it's a known placeholder
  if (cleanRef.toUpperCase() === 'LAST_UPLOADED' || cleanRef.toUpperCase() === 'ULTIMA_IMAGEM') {
    const last = localStorage.getItem('void_last_uploaded_img');
    if (last) return last;
  }
  
  // 3. Local storage references with smart lookup
  ...
  // 4. Case-insensitive search as a final attempt for IDs
  const resolvedFull = tryGet(cleanRef);
  if (resolvedFull) return resolvedFull;

  // 5. Final fallback (Avoid returning plain text like 'Icon')
...
const WELCOME_MESSAGES = [
  <>Saudações, Operativo. Você ancorou na Nexus, o <span className="text-cyan-300 font-black drop-shadow-[0_0_8px_rgba(103,232,249,0.3)]">limiar da criação absoluta</span>. Onde a estática termina, sua lenda começa. Sincronize seu link com a infinitude.</>,
